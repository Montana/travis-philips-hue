...
from node_types import (
    HueDimmLight,
    HueWhiteLight,
    HueColorLight, # Red (Fail)
    HueEColorLight, # Green (Pass)
    HueGroup,
)
import sys
import socket
    ...
    def connect(self):
        ...
        if "customData" in self.polyConfig:
            if "bridge_ip" in self.polyConfig["customData"]:
                ...
                custom_data_ip = True
                LOGGER.info(
                    "Bridge IP found in the Database: {}".format(bridge_ip))
            ...
            if "bridges" in self.polyConfig["customData"]:
                for idx, bridge in self.polyConfig["customData"][
                        "bridges"].items():
                    bridges[bridge["ip"]] = bridge["user"]
                LOGGER.info("Database has {} bridge(s) configuration".format(
                    len(bridges)))
            else:
        ...
        if "bridges" in self.polyConfig["customParams"]:
            try:
                hub_list = json.loads(
                    self.polyConfig["customParams"]["bridges"])
            except Exception as ex:
                LOGGER.error("Failed to read bridges variable {} {}".format(
                    self.polyConfig["customParams"]["bridges"], ex))
                return
            ...
            if len(bridges) > 0:
                ...
                LOGGER.info(
                    "No bridge configuration found, trying discovery...")
                hub_list = [None]
        ...
        for hub_ip in hub_list:
            ...
            if hub_ip in bridges:
                LOGGER.info(
                    "Found username for bridge {} in the DB".format(hub_ip))
            ...
            try:
                ...
                self.addNotice({
                    "myNotice":
                    "Please press the button on the Hue Bridge(s) and restart the node server within 30 seconds"
                })
                continue
            except Exception:
                ...
                if self.lights[hub_ip]:
                    ...
                    LOGGER.error(
                        "Connect: Failed to read Lights from the Hue Bridge")
